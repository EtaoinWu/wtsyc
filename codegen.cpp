...
namespace SysY::Eeyore {
  ...
  F::String outputDecl(const DeclarationX &dec) {
    return F::string(
        std::visit(overloaded{[](const DeclarationS &dec) {
                                return fmt::format("var {}", varName(dec.name));
                              },
                              [](const DeclarationA &dec) {
                                return fmt::format("var {} {}", dec.len,
                                                   varName(dec.name));
                              }},
                   dec)
            .c_str());
  ...
  F::String outputStmt(const Statement &stmt) {
    return F::string(
        std::visit(
            overloaded{
                [](const ExprU &p) {
                  return fmt::format("{} = {} {}", varName(p.dst),
                                     toString(p.op), operandName(p.src));
                },
                [](const ExprB &p) {
                  return fmt::format("{} = {} {} {}", varName(p.dst),
                                     operandName(p.src1), toString(p.op),
                                     operandName(p.src2));
                },
                [](const ExprC &p) {
                  return fmt::format("{} = {}", varName(p.dst),
                ...
                [](const ExprAW &p) {
                  return fmt::format("{}[{}] = {}", varName(p.dst_b),
                                     operandName(p.dst_i), operandName(p.src));
                },
                [](const ExprAR &p) {
                  return fmt::format("{} = {}[{}]", varName(p.dst),
                                     varName(p.src_b), operandName(p.src_i));
                },
